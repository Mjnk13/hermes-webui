  ...
  try{
    ...
    try{
      const refreshUrl=(typeof _sessionDisplayRefreshUrl==='function')
        ?_sessionDisplayRefreshUrl(sid)
        :`/api/session?session_id=${encodeURIComponent(sid)}&messages=1&resolve_model=0&msg_limit=30&expand_renderable=1`;
      const live=await api(refreshUrl,{timeoutMs:120000});
      ...
      if(typeof _msgLimitMax!=='undefined'){
        ...
        _msgLimitMax=live.session._msg_limit_max||fallbackMax;
      }
    }catch(preflightErr){
      if(typeof clearCompressionUi==='function') clearCompressionUi();
      if(typeof _setCompressionSessionLock==='function') _setCompressionSessionLock(null);
