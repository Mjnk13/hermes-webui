...
function _artifactToolCallHasConfirmedMutation(tc, preview){
  if(!tc||typeof tc!=='object') return false;
  const name=String(tc.name||'').replace(/^functions\./,'');
  if(ARTIFACT_MUTATION_TOOLS.has(name)) return true;
  const meta=preview&&typeof preview==='object'?preview:{};
  if(meta.confirmed_edit===true||meta.confirmedEdit===true||meta.mutated===true||meta.mutation===true) return true;
  const operation=String(meta.operation||meta.action||meta.kind||'').trim().toLowerCase();
  if(/^(?:write|written|create|created|edit|edited|patch|patched|delete|deleted|remove|removed|rename|renamed|move|moved)$/.test(operation)) return true;
  return Array.isArray(meta.files)&&meta.files.some(file=>{
    if(!file||typeof file!=='object') return false;
    if(file.confirmed_edit===true||file.confirmedEdit===true||file.mutated===true||file.mutation===true) return true;
    const fileOperation=String(file.operation||file.action||file.kind||'').trim().toLowerCase();
    if(/^(?:write|written|create|created|edit|edited|patch|patched|delete|deleted|remove|removed|rename|renamed|move|moved)$/.test(fileOperation)) return true;
    const candidate=file.diff||file.patch||'';
    return !!candidate&&_workspaceDiffLooksUseful(String(candidate));
  });
}
...
function _artifactCandidatesFromToolCall(tc){
  ...
  const add = (path, source=name || 'tool') => {
    ...
    if(path&&!seen.has(path)){
      seen.add(path);
      out.push({path, kind:source});
    }
  };
  if(preview && Array.isArray(preview.files) && _artifactToolCallHasConfirmedMutation({name,args,result},preview)){
  ...
  if(ARTIFACT_MUTATION_TOOLS.has(name) && args && typeof args === 'object'){
    ...
    for(const key of ['patch','diff']){
      if(typeof args[key]==='string') for(const a of _artifactCandidatesFromText(args[key])) add(a.path,a.kind);
    }
  }
  ...
  for(const a of _artifactCandidatesFromText(resultText)) add(a.path,a.kind);
  if(!out.length && ARTIFACT_MUTATION_TOOLS.has(name)){
    const argsText = typeof args === 'string' ? args : JSON.stringify(args || {});
    for(const a of _artifactCandidatesFromText(argsText)) add(a.path,a.kind);
  }
  return out;
}
...
function assistantMessageModifiedFiles(msg, rawIdx){
  ...
  const pushToolCall=(tc)=>{
    ...
    if(!fakeTc) return;
    const preview=fakeTc.mutation_preview || fakeTc.mutationPreview || {};
    if(preview && Array.isArray(preview.files) && _artifactToolCallHasConfirmedMutation(fakeTc,preview)){
...
function _workspaceNormalizeOpenArtifactPath(path){
  ...
  const normalizedAttempted=attempted.replace(/\\/g,'/');
  const ws=S&&S.session&&S.session.workspace?String(S.session.workspace).replace(/\\/g,'/').replace(/\/+$/,''):'';
  if(!S.session) return {attempted,rel:'',reason:'workspace not selected'};
  if(!ws) return {attempted,rel:'',reason:'workspace not selected'};
  let rel=normalizedAttempted.replace(/^~\//,'').replace(/^\.\/+/,'');
...
function _showWorkspaceOpenFileDiagnostic(attemptedPath, reason){
  ...
  // Keep the detailed reason visible in the error toast, while restoring the
  // stable status code consumed by session recovery and other UI owners.
  if(typeof setStatus==='function') setStatus(message);
  if(typeof showToast==='function') showToast(message,6500,'error');
  if(typeof setStatus==='function') setStatus(typeof t==='function'?t('file_open_failed'):'file_open_failed');
}
