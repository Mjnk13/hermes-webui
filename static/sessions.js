...
async function loadSession(sid){
  ...
  if (currentSid !== sid || forceReload) {
    ...
    if (currentSid && currentSid !== sid && typeof closeOtherLiveStreams === 'function') {
      ...
      // blocks the browser renderer during every sidebar selection.
...
// Display-only refreshes (soft reconnect, terminal recovery, retry/undo) must
// never fall back to the bare /api/session contract: that endpoint intentionally
// returns the complete transcript for operations such as export.  A long-lived
// session can make that response tens of megabytes and replace the paginated
// pane with thousands of rows.  Keep the currently visible width when practical,
// but clamp it to the server-advertised display ceiling so older rows remain
// reachable through the existing backward-pagination path.
function _sessionDisplayRefreshUrl(sid){
  const advertisedMax=Math.max(1,Number(_msgLimitMax)||_MSG_LIMIT_MAX);
  const loadedRenderableCount=_currentLoadedRenderableMessageCount();
  const limit=Math.min(
    advertisedMax,
    Math.max(_INITIAL_MSG_LIMIT,loadedRenderableCount),
  );
  return `/api/session?session_id=${encodeURIComponent(sid)}&messages=1&resolve_model=0&msg_limit=${limit}&expand_renderable=1`;
}
