(function(){
  ...
  function renderBrowserWorkbenchFrame(tab,host){
    ...
    if(existing){
      // A suspended/restored sibling may have replaced the viewport with a
      // placeholder while this tab's iframe stayed parked in the same host.
      // Clear that transient sibling content before revealing the preserved
      // surface again; otherwise the restored-history message remains on top
      // of an already-loaded page after an A -> B -> A tab switch.
      clearBrowserWorkbenchHostForRender(targetHost);
  ...
  function browserWorkbenchPreferredClientRenderer(){
    return workbenchCapabilities.native_devtools===true&&workbenchCapabilities.interactive_viewport===true
      ?'chromium-stream'
      :'iframe-bridge';
  }
  ...
  function browserWorkbenchRequestBody(extra){
    return JSON.stringify({
      ...
      client_renderer:browserWorkbenchPreferredClientRenderer(),
      ...(extra||{})
    });
  }
  ...
  async function closeBrowserWorkbenchTab(tabId){
    ...
    const target=requestedTabId?tabById(requestedTabId):getActiveWorkbenchTab();
    ...
    if(activeBrowserWorkbenchTabId){
      ...
      if(typeof switchPanel==='function'){
        try{Promise.resolve(switchPanel('browser')).catch(()=>{});}catch(_){}
      }
    }else{
      renderActiveBrowserWorkbenchView();
      if(typeof switchPanel==='function'){
        try{Promise.resolve(switchPanel('chat')).catch(()=>{});}catch(_){}
      }
    }
    persistBrowserWorkbenchTabs();
    if(closingId){
      try{
        Promise.resolve(requestJSON(sessionStatusUrl(closingId),{method:'DELETE'})).catch((err)=>{
          if(isBrowserWorkbenchSessionMissingError(err))return;
          if(typeof showToast==='function')showToast('Tab closed, but browser cleanup failed.');
        });
      }catch(_){
        if(typeof showToast==='function')showToast('Tab closed, but browser cleanup failed.');
      }
