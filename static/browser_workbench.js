(function(){
  ...
  let browserWorkbenchFindOpenTabId='';
  ...
  function currentNativeBoundsPayload(tab,visible){
    ...
    const style=rect&&window.getComputedStyle?window.getComputedStyle(viewportEl):null;
    const borderLeft=Math.max(0,parseFloat(style&&style.borderLeftWidth)||0);
    const borderTop=Math.max(0,parseFloat(style&&style.borderTopWidth)||0);
    const borderRight=Math.max(0,parseFloat(style&&style.borderRightWidth)||0);
    const borderBottom=Math.max(0,parseFloat(style&&style.borderBottomWidth)||0);
    const borderRadius=Math.max(0,Math.round(parseFloat(style&&style.borderTopLeftRadius)||0));
    const activePanel=document.querySelector('main.main.showing-browser');
    let bounds=rect?{
      x:Math.round(rect.left+borderLeft),
      y:Math.round(rect.top+borderTop),
      width:Math.max(0,Math.round(rect.width-borderLeft-borderRight)),
      height:Math.max(0,Math.round(rect.height-borderTop-borderBottom))
  ...
  function updateBrowserWorkbenchFindUi(tab){
    const target=tab||tabById(browserWorkbenchFindOpenTabId);
    const query=String(target&&target.findQuery||'');
    const matches=Math.max(0,Number(target&&target.findMatches)||0);
    const active=Math.max(0,Number(target&&target.findActiveMatch)||0);
    const searching=target&&target.findFinalUpdate===false;
    if(findMatchesEl)findMatchesEl.textContent=searching?'…':`${active}/${matches}`;
    if(findBar)findBar.dataset.empty=query&&target&&target.findFinalUpdate===true&&matches===0?'true':'false';
    if(findPreviousButton)findPreviousButton.disabled=!query;
    if(findNextButton)findNextButton.disabled=!query;
  }

  function runBrowserWorkbenchFind(options){
    const opts=options&&typeof options==='object'?options:{};
    const active=getActiveWorkbenchTab();
    if(!active||active.id!==browserWorkbenchFindOpenTabId||active.renderer!=='electron-native'||!active.sessionId)return Promise.resolve(null);
    const query=String(findInput&&findInput.value||'');
    active.findQuery=query;
    active.findFinalUpdate=false;
    if(!query){
      active.findMatches=0;
      active.findActiveMatch=0;
      active.findFinalUpdate=true;
      updateBrowserWorkbenchFindUi(active);
      return callDesktopBrowserBridge('stopFindInPage',{sessionId:active.sessionId,focusPage:false});
    }
    updateBrowserWorkbenchFindUi(active);
    return callDesktopBrowserBridge('findInPage',{
      sessionId:active.sessionId,
      query,
      forward:opts.forward!==false,
      findNext:opts.startNew===true,
    }).catch((err)=>{
      active.findMatches=0;
      active.findActiveMatch=0;
      active.findFinalUpdate=true;
      updateBrowserWorkbenchFindUi(active);
      setStatus('Find in page failed.','warning',active,{owner:'find',kind:'error'});
      return null;
    });
  }

  function openBrowserWorkbenchFind(payload){
    wireDom();
    const detail=payload&&typeof payload==='object'?payload:{};
    const active=getActiveWorkbenchTab();
    if(!active||active.renderer!=='electron-native'||!active.sessionId)return false;
    ...
    browserWorkbenchFindOpenTabId=active.id;
    if(findBar)findBar.hidden=false;
    if(findInput)findInput.value=String(active.findQuery||'');
    updateBrowserWorkbenchFindUi(active);
    requestAnimationFrame(()=>{
      if(active.id!==browserWorkbenchFindOpenTabId||!findInput)return;
      findInput.focus();
      findInput.select();
      if(findInput.value)void runBrowserWorkbenchFind({startNew:true});
    });
    return true;
  }

  function closeBrowserWorkbenchFind(options){
    wireDom();
    const opts=options&&typeof options==='object'?options:{};
    const target=tabById(opts.tabId||browserWorkbenchFindOpenTabId);
    if(!target)return false;
    const wasOpen=browserWorkbenchFindOpenTabId===target.id;
    if(wasOpen){
      browserWorkbenchFindOpenTabId='';
      if(findBar)findBar.hidden=true;
    }
    target.findMatches=0;
    target.findActiveMatch=0;
    target.findFinalUpdate=true;
    browserWorkbenchClearStatus(target,{owner:'find'});
    if(target.renderer==='electron-native'&&target.sessionId){
      const payload=opts.focusPage===false
        ?{sessionId:target.sessionId,focusPage:false}
        :{sessionId:target.sessionId,focusPage:true};
      callDesktopBrowserBridge('stopFindInPage',payload).catch(()=>{});
    }
    return wasOpen;
  }
  ...
  function wireDom(){
    ...
    findBar=document.getElementById('browserWorkbenchFindBar');
    findInput=document.getElementById('browserWorkbenchFind');
    findMatchesEl=document.getElementById('browserWorkbenchFindMatches');
    findPreviousButton=document.getElementById('browserWorkbenchFindPrevious');
    findNextButton=document.getElementById('browserWorkbenchFindNext');
    findCloseButton=document.getElementById('browserWorkbenchFindClose');
    viewportEl=document.getElementById('browserWorkbenchViewport');
    if(findInput&&!findInput.dataset.browserWorkbenchWired){
      findInput.dataset.browserWorkbenchWired='1';
      findInput.addEventListener('input',()=>{void runBrowserWorkbenchFind({startNew:true});});
      findInput.addEventListener('keydown',(event)=>{
        ...
        if(key==='f'&&(event.metaKey||event.ctrlKey)){
          ...
          event.preventDefault();
          void runBrowserWorkbenchFind({forward:!event.shiftKey,startNew:false});
        }else if(event.key==='Escape'){
          event.preventDefault();
          event.stopPropagation();
          closeBrowserWorkbenchFind({focusPage:true});
        }
      });
    }
    if(findPreviousButton&&!findPreviousButton.dataset.browserWorkbenchWired){
      findPreviousButton.dataset.browserWorkbenchWired='1';
      findPreviousButton.addEventListener('click',()=>{void runBrowserWorkbenchFind({forward:false,startNew:false});});
    }
    if(findNextButton&&!findNextButton.dataset.browserWorkbenchWired){
      findNextButton.dataset.browserWorkbenchWired='1';
      findNextButton.addEventListener('click',()=>{void runBrowserWorkbenchFind({forward:true,startNew:false});});
    }
    if(findCloseButton&&!findCloseButton.dataset.browserWorkbenchWired){
      findCloseButton.dataset.browserWorkbenchWired='1';
      findCloseButton.addEventListener('click',()=>closeBrowserWorkbenchFind({focusPage:true}));
    }
    ...
    if(!document.documentElement.dataset.browserWorkbenchNativeFindWired&&window.hermesDesktop&&window.hermesDesktop.browser){
      const bridge=window.hermesDesktop.browser;
      if(typeof bridge.onFindRequested==='function'&&typeof bridge.onFindResult==='function'){
        document.documentElement.dataset.browserWorkbenchNativeFindWired='1';
        bridge.onFindRequested((payload)=>{openBrowserWorkbenchFind(payload);});
        bridge.onFindResult((payload)=>{
          const detail=payload&&typeof payload==='object'?payload:{};
          const active=getActiveWorkbenchTab();
          if(!active||active.id!==browserWorkbenchFindOpenTabId)return;
          const sessionId=String(detail.session_id||detail.sessionId||'');
          if(sessionId&&sessionId!==active.sessionId)return;
          active.findMatches=Math.max(0,Number(detail.matches)||0);
          active.findActiveMatch=Math.max(0,Number(detail.active_match_ordinal||detail.activeMatchOrdinal)||0);
          active.findFinalUpdate=detail.final_update===true||detail.finalUpdate===true;
          updateBrowserWorkbenchFindUi(active);
        });
      }
    }
  ...
  function applyBrowserWorkbenchNativeNavigationUpdate(payload){
    ...
    // A main-frame navigation destroys the inspected document. End Ping mode
    // explicitly so its overlay is not injected into the replacement document
    // by the native did-finish-load handler.
    if(detail.reason==='did-start-navigation'&&selectionModeTabId===target.id){
      setBrowserWorkbenchSelectionMode(false);
    }
    if(detail.reason==='did-start-navigation'&&browserWorkbenchFindOpenTabId===target.id){
      closeBrowserWorkbenchFind({tabId:target.id,focusPage:false});
    }
  ...
  function activateBrowserWorkbenchTab(tabId,opts){
    ...
    if(previous&&previous.id!==target.id){
      if(browserWorkbenchFindOpenTabId===previous.id)closeBrowserWorkbenchFind({tabId:previous.id,focusPage:false});
  ...
  async function closeBrowserWorkbenchTab(tabId){
    ...
    if(browserWorkbenchFindOpenTabId===target.id)closeBrowserWorkbenchFind({tabId:target.id,focusPage:false});
    const closingId=target.sessionId;
    if(closingId){
      setTabState('loading',target);
  ...
  function syncBrowserWorkbenchTabActive(activePanel){
    ...
    if(isBrowser)renderActiveBrowserWorkbenchView();
      ...
      const active=getActiveWorkbenchTab();
      if(browserWorkbenchFindOpenTabId===activeBrowserWorkbenchTabId)closeBrowserWorkbenchFind({tabId:activeBrowserWorkbenchTabId,focusPage:false});
      if(selectionMode)setBrowserWorkbenchSelectionMode(false);
