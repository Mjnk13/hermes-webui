...
def _filter_reasoning_efforts_for_provider(
    ...
    # Hermes also maps canonical ``ultra`` to the provider wire value ``max``.
...
def _load_credential_pool_for_active_home(loader, provider_id: str):
    """Load an Agent credential pool under WebUI's active auth-store home.

    Newer Hermes Agent builds resolve auth.json through a context-local home
    override.  Installing it here keeps direct WebUI catalog/provider probes
    profile-safe even when they run outside a request worker (and avoids any
    fallback to the process user's real auth store during tests).
    """
    override_mod = None
    override_token = None
    try:
        import hermes_constants as override_mod

        if hasattr(override_mod, "set_hermes_home_override"):
            override_token = override_mod.set_hermes_home_override(
                _get_auth_store_path().parent
            )
    except Exception:
        override_mod = None
        override_token = None
    try:
        ...
        if (
            override_mod is not None
            and override_token is not None
            and hasattr(override_mod, "reset_hermes_home_override")
        ):
            try:
                override_mod.reset_hermes_home_override(override_token)
            except Exception:
                logger.debug("Failed to reset credential-pool Hermes-home override")
    ...
    try:
        ...
        if _cached is not None:
            ...
            if (time.time() - _cp_ts) < 86400.0:
                ...
                _cp_pool = _load_credential_pool_for_active_home(_load_pool, _pid)
