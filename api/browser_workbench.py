...
def _browser_proxy_timeout_for_target(target_url: str) -> int:
    """Allow bounded local-dev cold starts without relaxing remote fetches."""
    try:
        hostname = urlsplit(target_url).hostname
    except ValueError:
        return _BROWSER_PROXY_TIMEOUT_SECONDS
    return (
        _BROWSER_PROXY_LOOPBACK_TIMEOUT_SECONDS
        if _is_loopback_browser_host(hostname)
        else _BROWSER_PROXY_TIMEOUT_SECONDS
    )
...
def _chii_css_editing_compat_script() -> str:
    """Augment Chobitsu's partial CSS domain with editable rule payloads.

    Chobitsu 1.8.6 intentionally implements ``CSS.setStyleTexts`` for inline
    styles only and walks only the top-level CSSRuleList. Chrome DevTools needs
    source ranges and declaration text to edit a matched stylesheet rule. This
    runtime-local adapter fills that protocol gap without altering globals in
    the inspected application or persisting edits beyond the live page.
    """
    return r"""
;(() => {
  'use strict';
  const patchKey = '__HERMES_CHII_CSS_EDITING_V1__';
  if (window[patchKey]) return;
  const chobitsu = window.chii && window.chii.chobitsu;
  if (!chobitsu || typeof chobitsu.domain !== 'function' || typeof chobitsu.register !== 'function') return;
  const cssDomain = chobitsu.domain('CSS');
  const domDomain = chobitsu.domain('DOM');
  if (!cssDomain || !domDomain || typeof domDomain.getNode !== 'function') return;
  window[patchKey] = true;

  const originalGetMatchedStylesForNode = typeof cssDomain.getMatchedStylesForNode === 'function'
    ? cssDomain.getMatchedStylesForNode.bind(cssDomain) : null;
  const originalGetStyleSheetText = typeof cssDomain.getStyleSheetText === 'function'
    ? cssDomain.getStyleSheetText.bind(cssDomain) : null;
  const originalSetStyleTexts = typeof cssDomain.setStyleTexts === 'function'
    ? cssDomain.setStyleTexts.bind(cssDomain) : null;
  const anonymousSheetIds = new WeakMap();
  const ruleLinesBySheet = new WeakMap();
  let anonymousSheetSequence = 0;
  let snapshots = new Map();

  const rangeKey = range => range ? [range.startLine, range.startColumn, range.endLine, range.endColumn].join(':') : '';
  const sameRange = (left, right) => Boolean(left && right && rangeKey(left) === rangeKey(right));
  ...
  const splitSelectorList = text => {
    ...
    for (let index = 0; index < raw.length; index += 1) {
      ...
      if (char === '(') round += 1;
        ...
        const selector = raw.slice(start, index).trim();
    ...
    return selectors.length ? selectors : [raw.trim()].filter(Boolean);
  };
  ...
  const styleEntries = style => {
    ...
    for (let index = 0; index < Number(style.length || 0); index += 1) {
      const name = String(style[index] || '').trim();
      if (!name) continue;
      const value = String(typeof style.getPropertyValue === 'function' ? style.getPropertyValue(name) : style[name] || '').trim();
      if (!value) continue;
      const priority = String(typeof style.getPropertyPriority === 'function' ? style.getPropertyPriority(name) : '').trim().toLowerCase();
      entries.push({name, value, important: priority === 'important'});
    }
    return entries;
  };
  const declarationFor = style => styleEntries(style).map(entry =>
    `${entry.name}: ${entry.value}${entry.important ? ' !important' : ''};`
  ).join(' ');

  const sheetIdFor = (sheet, index) => {
    if (sheet && sheet.styleSheetId) return String(sheet.styleSheetId);
    let generated = sheet && anonymousSheetIds.get(sheet);
    if (!generated) {
      generated = `hermes-chii-sheet-${index}-${++anonymousSheetSequence}`;
      if (sheet) anonymousSheetIds.set(sheet, generated);
    }
    try { if (sheet && !sheet.styleSheetId) sheet.styleSheetId = generated; } catch (_) {}
    return generated;
  };

  const groupingRuleIsActive = rule => {
    try {
      if (typeof CSSMediaRule !== 'undefined' && rule instanceof CSSMediaRule) {
        const query = String(rule.conditionText || (rule.media && rule.media.mediaText) || '');
      ...
      if (typeof CSSSupportsRule !== 'undefined' && rule instanceof CSSSupportsRule) {
        ...
        return !condition || !window.CSS || typeof window.CSS.supports !== 'function' || window.CSS.supports(condition);
      }
    } catch (_) {}
    return true;
  };

  const stableRuleLine = (sheet, rule) => {
    let state = ruleLinesBySheet.get(sheet);
    if (!state) {
      state = {lines: new WeakMap(), nextLine: 0};
      ruleLinesBySheet.set(sheet, state);
    }
    let line = state.lines.get(rule);
    if (line === undefined) {
      line = state.nextLine++;
      state.lines.set(rule, line);
    }
    return line;
  };
  ...
  const rebuildSnapshots = () => {
    ...
    sheets.forEach((sheet, sheetIndex) => {
      const styleSheetId = sheetIdFor(sheet, sheetIndex);
      const snapshot = {styleSheetId, sheet, text: '', declarations: []};
      const textLines = [];
      let rules = [];
      ...
      const walk = ruleList => {
        for (const rule of Array.from(ruleList || [])) {
          if (rule && rule.selectorText && rule.style) {
            ...
            const declarationText = declarationFor(rule.style);
            const prefix = `${selectorText} { `;
            const line = stableRuleLine(sheet, rule);
            textLines[line] = prefix + declarationText + ' }';
            const declaration = {
              rule,
              selectorText,
              declarationText,
              ...
              range: {
                ...
                endLine: line,
                endColumn: prefix.length + declarationText.length,
              },
            };
            snapshot.declarations.push(declaration);
          }
          try { if (rule && rule.cssRules && groupingRuleIsActive(rule)) walk(rule.cssRules); } catch (_) {}
        }
      };
      walk(rules);
      snapshot.text = textLines.map(line => line || '').join('\n');
      if (textLines.length) snapshot.text += '\n';
      next.set(styleSheetId, snapshot);
    });
    snapshots = next;
    return snapshots;
  };

  const stylePayload = (snapshot, declaration) => {
    const entries = styleEntries(declaration.rule.style);
    const propertyTexts = entries.map(entry =>
      `${entry.name}: ${entry.value}${entry.important ? ' !important' : ''};`
    );
    const declarationText = propertyTexts.join(' ');
    let cursor = declaration.range.startColumn;
    const cssProperties = entries.map((entry, index) => {
      const text = propertyTexts[index];
      const property = {
        name: entry.name,
        value: entry.value,
        text,
        range: {
          ...
          startColumn: cursor,
          endLine: declaration.range.startLine,
          endColumn: cursor + text.length,
        },
        parsedOk: true,
        disabled: false,
        implicit: false,
      };
      if (entry.important) property.important = true;
      cursor += text.length + (index < entries.length - 1 ? 1 : 0);
      return property;
    });
    return {
  ...
  const ruleMatch = (node, snapshot, declaration) => {
    const selectors = splitSelectorList(declaration.selectorText);
    const matchingSelectors = [];
    selectors.forEach((selector, index) => {
      try { if (node.matches(selector)) matchingSelectors.push(index); } catch (_) {}
    });
    if (!matchingSelectors.length) return null;
    return {
      matchingSelectors,
      rule: {
        styleSheetId: declaration.styleSheetId,
        selectorList: {text: declaration.selectorText, selectors: selectors.map(text => ({text}))},
        origin: 'regular',
        style: stylePayload(snapshot, declaration),
      },
    };
  };

  const getMatchedStylesForNode = params => {
    let base = {};
    try { base = originalGetMatchedStylesForNode ? originalGetMatchedStylesForNode(params) || {} : {}; } catch (_) {}
    let node = null;
    try { node = (domDomain.getNode(params) || {}).node || null; } catch (_) {}
    if (!node || typeof node.matches !== 'function') return base;
    const matchedCSSRules = [];
    for (const snapshot of rebuildSnapshots().values()) {
      for (const declaration of snapshot.declarations) {
        const matched = ruleMatch(node, snapshot, declaration);
        if (matched) matchedCSSRules.push(matched);
      }
    }
    return Object.assign({}, base, {matchedCSSRules});
  };

  const getStyleSheetText = async params => {
    const styleSheetId = String(params && params.styleSheetId || '');
    const snapshot = snapshots.get(styleSheetId) || rebuildSnapshots().get(styleSheetId);
    if (snapshot) return {text: snapshot.text};
    return originalGetStyleSheetText ? await originalGetStyleSheetText(params) : {text: ''};
  };

  const setStyleTexts = async params => {
    const edits = Array.isArray(params && params.edits) ? params.edits : [];
    const styles = [];
    for (const edit of edits) {
      const styleSheetId = String(edit && edit.styleSheetId || '');
      let snapshot = rebuildSnapshots().get(styleSheetId);
      let declaration = snapshot && snapshot.declarations.find(item => sameRange(item.range, edit.range));
      if (snapshot && declaration && declaration.rule && declaration.rule.style) {
        let text = String(edit.text || '').trim();
        if (text.startsWith('{') && text.endsWith('}')) text = text.slice(1, -1).trim();
        declaration.rule.style.cssText = text;
        snapshot = rebuildSnapshots().get(declaration.styleSheetId);
        declaration = snapshot && snapshot.declarations.find(item => item.rule === declaration.rule);
        styles.push(declaration ? stylePayload(snapshot, declaration) : {styleSheetId});
        continue;
      }
      if (originalSetStyleTexts) {
        const fallback = await originalSetStyleTexts(Object.assign({}, params, {edits: [edit]}));
        styles.push(fallback && Array.isArray(fallback.styles) && fallback.styles[0]
          ? fallback.styles[0] : {styleSheetId: String(edit && edit.styleSheetId || '')});
      } else {
        styles.push({styleSheetId: String(edit && edit.styleSheetId || '')});
      }
    }
    return {styles};
  };

  chobitsu.register('CSS', {getMatchedStylesForNode, getStyleSheetText, setStyleTexts});
})();
"""
...
def _chii_target_runtime_script() -> bytes:
    ...
    script += _chii_css_editing_compat_script()
    ...
    *,
    ...
    def attr_repl(match, tag_name: str, opaque_iframe: bool):
        ...
        if normalized_attr in {"action", "formaction"} or (
            ...
            return match.group(0)
        # Passive media can load cross-origin exactly as it does in a native
        # browser. Keep framework-owned SSR attributes byte-for-byte stable so
        # React/Next hydration does not compare an original URL with a Hermes
        # transport URL. Root-relative requests are recovered from the live
        # proxy referrer; fully sandboxed opaque frames still require explicit
        # proxy URLs because they cannot participate in that recovery path.
        if not explicit_resource_url and (
            normalized_attr == "data-src"
            or normalized_attr in passive_resource_attributes.get(normalized_tag, set())
        ):
            return match.group(0)
    ...
    # Inline style attributes and <style> text are also framework-owned SSR
    # state. Preserve them in ordinary proxy documents; passive external media
    # loads directly and root-relative media uses referrer recovery. Opaque
    # sandbox documents keep the explicit rewrite required by their isolation.
    if rewrite_root_relative:
...
(() => {
  ...
  const resolveBrowserWorkbenchBlobUrl = (value) => {
    try {
      if (value === undefined || value === null) return value;
      return browserWorkbenchBlobUrlAliases.get(String(value)) || value;
    } catch (_) { return value; }
  };
  const exposeBrowserWorkbenchBlobUrl = (value) => {
    try {
      if (value === undefined || value === null) return value;
      return browserWorkbenchBlobUrlDisplayAliases.get(String(value)) || value;
    } catch (_) { return value; }
  };
  const replaceBrowserWorkbenchBlobUrls = (value, replacements) => {
    try {
      if (value === undefined || value === null || replacements.size === 0) return value;
      const raw = String(value);
      if (!raw.includes('blob:')) return value;
      let replaced = raw;
      replacements.forEach((replacement, candidate) => {
        if (replaced.includes(candidate)) replaced = replaced.split(candidate).join(replacement);
      });
      return replaced === raw ? value : replaced;
    } catch (_) { return value; }
  };
  const resolveBrowserWorkbenchBlobText = (value) => {
    return replaceBrowserWorkbenchBlobUrls(value, browserWorkbenchBlobUrlAliases);
  };
  const exposeBrowserWorkbenchBlobText = (value) => {
    return replaceBrowserWorkbenchBlobUrls(value, browserWorkbenchBlobUrlDisplayAliases);
  };
  const shouldExposeTargetObjectUrl = (blob) => {
    try {
      if (
        (typeof window.File === 'function' && blob instanceof window.File)
        || Object.prototype.toString.call(blob) === '[object File]'
      ) return true;
      const contentType = String(blob && blob.type || '').split(';', 1)[0].trim().toLowerCase();
      return !/(?:javascript|ecmascript)|^text\/(?:jscript|livescript)$/.test(contentType);
    } catch (_) {
      return true;
    }
  };
  const browserWorkbenchObjectUrlToken = () => {
    try {
  ...
  const installTargetObjectUrlShim = () => {
    try {
      if (window.__HERMES_BROWSER_WORKBENCH_OBJECT_URL_SHIM__) return true;
      const UrlCtor = window.URL;
      const createDescriptor = UrlCtor && Object.getOwnPropertyDescriptor(UrlCtor, 'createObjectURL');
      const revokeDescriptor = UrlCtor && Object.getOwnPropertyDescriptor(UrlCtor, 'revokeObjectURL');
      if (!createDescriptor || typeof createDescriptor.value !== 'function' || !revokeDescriptor || typeof revokeDescriptor.value !== 'function') return false;
      const nativeCreateObjectURL = createDescriptor.value.bind(UrlCtor);
      const nativeRevokeObjectURL = revokeDescriptor.value.bind(UrlCtor);
      Object.defineProperty(UrlCtor, 'createObjectURL', Object.assign({}, createDescriptor, {
        value(blob) {
          const nativeUrl = String(nativeCreateObjectURL(blob));
          if (!shouldExposeTargetObjectUrl(blob)) return nativeUrl;
          let publicUrl = '';
          for (let attempt = 0; attempt < 8 && !publicUrl; attempt += 1) {
            const token = browserWorkbenchObjectUrlToken();
            if (!token) break;
            const candidate = `blob:${targetOrigin}/${token}`;
            if (!browserWorkbenchBlobUrlAliases.has(candidate)) publicUrl = candidate;
          }
          if (!publicUrl) {
            nativeRevokeObjectURL(nativeUrl);
            throw new Error('Could not allocate a target-scoped object URL.');
          }
          browserWorkbenchBlobUrlAliases.set(publicUrl, nativeUrl);
          browserWorkbenchBlobUrlSources.set(nativeUrl, publicUrl);
          browserWorkbenchBlobUrlDisplayAliases.set(nativeUrl, publicUrl);
          return publicUrl;
        }
      }));
      Object.defineProperty(UrlCtor, 'revokeObjectURL', Object.assign({}, revokeDescriptor, {
        value(value) {
          const raw = String(value === undefined || value === null ? '' : value);
          const nativeUrl = String(browserWorkbenchBlobUrlAliases.get(raw) || raw);
          const publicUrl = browserWorkbenchBlobUrlSources.get(nativeUrl) || (browserWorkbenchBlobUrlAliases.has(raw) ? raw : '');
          if (publicUrl) browserWorkbenchBlobUrlAliases.delete(publicUrl);
          browserWorkbenchBlobUrlSources.delete(nativeUrl);
          // Keep only the native/public strings for DOM/CSS getters that still
          // reflect a revoked URL. The browser owns and releases the Blob bytes;
          // this document-scoped display tombstone disappears on navigation.
          return nativeRevokeObjectURL(nativeUrl);
        }
      }));
      window.__HERMES_BROWSER_WORKBENCH_OBJECT_URL_SHIM__ = true;
      return true;
    } catch (_) {
      return false;
    }
  };
  installTargetObjectUrlShim();
  ...
  const toProxyHttp = (value, transportPathOverride) => {
    try {
      ...
      const resolvedBlobUrl = resolveBrowserWorkbenchBlobUrl(value);
      const raw = String(resolvedBlobUrl);
      const lower = raw.toLowerCase();
      if (lower.startsWith('blob:')) return resolvedBlobUrl;
      if (lower.startsWith('data:') || lower.startsWith('javascript:') || lower.startsWith('mailto:') || lower.startsWith('tel:') || raw.startsWith(proxyPrefix)) return value;
  ...
  const isBrowserWorkbenchBlobUrlAttribute = (element, name) => {
    try {
      const attribute = String(name || '').trim().toLowerCase();
      if (attribute === 'style') return true;
      const tag = String(element && (element.localName || element.tagName) || '').trim().toLowerCase();
      switch (tag) {
        case 'a':
        case 'area':
        case 'base':
        ...
        case 'input':
          return attribute === 'src' || attribute === 'formaction';
        case 'object':
  ...
  const installTargetBlobUrlConsumerBoundary = () => {
    try {
      const elementPrototype = window.Element && window.Element.prototype;
      if (!elementPrototype) return false;
      if (elementPrototype.__HERMES_TARGET_BLOB_URL_BOUNDARY__) return true;
      ...
      if (typeof originalSetAttribute === 'function') {
        elementPrototype.setAttribute = function(name, value) {
          const nextValue = isBrowserWorkbenchBlobUrlAttribute(this, name)
            ? resolveBrowserWorkbenchBlobText(value)
            : value;
          return originalSetAttribute.call(this, name, nextValue);
        };
      }
      if (typeof originalGetAttribute === 'function') {
        elementPrototype.getAttribute = function(name) {
          const value = originalGetAttribute.call(this, name);
          return isBrowserWorkbenchBlobUrlAttribute(this, name)
            ? exposeBrowserWorkbenchBlobText(value)
            : value;
        };
      }
        ...
        try {
          ...
          Object.defineProperty(prototype, property, {
            ...
            get:typeof descriptor.get === 'function'
              ? function() { return exposeBrowserWorkbenchBlobText(descriptor.get.call(this)); }
              : descriptor.get,
            set(value) { return descriptor.set.call(this, resolveBrowserWorkbenchBlobText(value)); }
          });
          return true;
      ...
      ].forEach(([constructorName, properties]) => {
        const constructor = window[constructorName];
        properties.forEach((property) => patchUrlProperty(constructor && constructor.prototype, property));
      });
      ...
      if (stylePrototype) {
        ...
        const originalGetPropertyValue = stylePrototype.getPropertyValue;
        if (typeof originalSetProperty === 'function') {
          stylePrototype.setProperty = function(name, value, priority) {
            return originalSetProperty.call(this, name, resolveBrowserWorkbenchBlobText(value), priority);
          };
        }
        if (typeof originalGetPropertyValue === 'function') {
          stylePrototype.getPropertyValue = function(name) {
            return exposeBrowserWorkbenchBlobText(originalGetPropertyValue.call(this, name));
          };
        }
        patchUrlProperty(stylePrototype, 'cssText');
        const patchCssUrlProperty = (property, cssName) => {
          if (patchUrlProperty(stylePrototype, property)) return;
          if (typeof originalSetProperty !== 'function' || typeof originalGetPropertyValue !== 'function') return;
          try {
            Object.defineProperty(stylePrototype, property, {
              configurable:true,
              enumerable:true,
              get() {
                return exposeBrowserWorkbenchBlobText(originalGetPropertyValue.call(this, cssName));
              },
              set(value) {
                return originalSetProperty.call(this, cssName, resolveBrowserWorkbenchBlobText(value), '');
              }
            });
          } catch (_) {}
        };
        ...
        ].forEach(([property, cssName]) => patchCssUrlProperty(property, cssName));
      }
      const styleSheetPrototype = window.CSSStyleSheet && window.CSSStyleSheet.prototype;
      const originalInsertRule = styleSheetPrototype && styleSheetPrototype.insertRule;
      if (typeof originalInsertRule === 'function') {
        styleSheetPrototype.insertRule = function(rule, index) {
          return originalInsertRule.call(this, resolveBrowserWorkbenchBlobText(rule), index);
        };
      }
      const originalReplace = styleSheetPrototype && styleSheetPrototype.replace;
      if (typeof originalReplace === 'function') {
        styleSheetPrototype.replace = function(text) {
          return originalReplace.call(this, resolveBrowserWorkbenchBlobText(text));
        };
      }
      const originalReplaceSync = styleSheetPrototype && styleSheetPrototype.replaceSync;
      if (typeof originalReplaceSync === 'function') {
        styleSheetPrototype.replaceSync = function(text) {
          return originalReplaceSync.call(this, resolveBrowserWorkbenchBlobText(text));
        };
      }
      ['Worker','SharedWorker','Audio'].forEach((constructorName) => {
        const Original = window[constructorName];
        if (typeof Original !== 'function') return;
        const Wrapped = function(url, ...args) {
          return Reflect.construct(Original, [resolveBrowserWorkbenchBlobUrl(url), ...args], Original);
        };
        try { Object.setPrototypeOf(Wrapped, Original); } catch (_) {}
        Wrapped.prototype = Original.prototype;
        try { window[constructorName] = Wrapped; } catch (_) {}
      });
      Object.defineProperty(elementPrototype, '__HERMES_TARGET_BLOB_URL_BOUNDARY__', {
        configurable:true,
        value:true
      });
      return true;
  ...
  installTargetBlobUrlConsumerBoundary();
  ...
  // A same-origin about:blank iframe is a special case for proxied pages:
  // browser requests from it do not carry the parent proxy document's
  // Referer, so the server-side clean-root recovery route cannot recover the
  // owning session. Install a frame-local resource boundary before the target
  // can append its first node. This preserves ordinary SSR attributes in the
  // outer document (and therefore framework hydration) while making resources
  // created inside a portal/blank preview explicit session-scoped proxy URLs.
  const nestedFrameStates = new WeakMap();
  let nestedFrameSequence = 0;
  const isSameOriginBlankFrame = (candidate) => {
    try {
      if (!candidate || String(candidate.tagName || '').toLowerCase() !== 'iframe') return false;
      if (isOpaqueSandboxedFrame(candidate)) return false;
      const raw = String(candidate.getAttribute('src') || '').trim().toLowerCase();
      return !raw || raw === 'about:blank' || raw.startsWith('about:blank#');
    } catch (_) { return false; }
  };
  const nestedFrameResourceAttribute = (element, attribute) => {
    try {
      const tag = String(element && (element.localName || element.tagName) || '').toLowerCase();
      const name = String(attribute || '').toLowerCase();
      if (name === 'style') return true;
      if (name === 'href') return ['link','image','use'].includes(tag);
      if (name === 'srcset') return ['img','source'].includes(tag);
      if (name === 'src') return ['audio','embed','iframe','img','input','script','source','track','video'].includes(tag);
      if (name === 'poster') return tag === 'video';
      if (name === 'data') return tag === 'object';
    } catch (_) {}
    return false;
  };
  const nestedFrameSetAttribute = (element, attribute, value, state) => {
    const raw = String(value || '').trim();
    if (!raw) return value;
    const name = String(attribute || '').toLowerCase();
    if (name === 'style') return proxyOpaqueCssText(raw, state.transportPath);
    if (name === 'srcset') {
      return raw.split(',').map((item) => {
        const parts = item.trim().split(/\s+/);
        if (parts[0]) parts[0] = toProxyHttp(parts[0], state.transportPath);
        return parts.join(' ');
      }).join(', ');
    }
    return toProxyHttp(raw, state.transportPath);
  };
  const nestedFrameWriteAttribute = (element, attribute, value, state) => {
    try {
      state.originalSetAttribute.call(element, attribute, value);
    } catch (_) {}
  };
  const rewriteNestedFrameNode = (node, state) => {
    try {
      if (!node || node.nodeType !== 1) return;
      ['href','src','srcset','poster','data','style'].forEach((attribute) => {
        if (!nestedFrameResourceAttribute(node, attribute) || !node.hasAttribute(attribute)) return;
        const current = node.getAttribute(attribute);
        const rewritten = nestedFrameSetAttribute(node, attribute, current, state);
        if (rewritten !== current) nestedFrameWriteAttribute(node, attribute, rewritten, state);
      });
      if (String(node.localName || '').toLowerCase() === 'style') {
        node.textContent = proxyOpaqueCssText(node.textContent || '', state.transportPath);
      }
      if (typeof node.querySelectorAll === 'function') {
        node.querySelectorAll('*').forEach((child) => rewriteNestedFrameNode(child, state));
      }
    } catch (_) {}
  };
  const installNestedFrameNetworkBoundary = (nestedWindow, state) => {
    try {
      if (!nestedWindow || nestedWindow.__HERMES_BROWSER_WORKBENCH_NESTED_RUNTIME__) return;
      const OriginalFetch = nestedWindow.fetch;
      if (typeof OriginalFetch === 'function') {
        nestedWindow.fetch = function(input, init) {
          let proxiedInput = input;
          try {
            if (typeof input === 'string' || input instanceof nestedWindow.URL) {
              proxiedInput = toProxyHttp(input.href || input, state.transportPath);
            } else if (input && typeof input.url === 'string' && typeof nestedWindow.Request === 'function') {
              proxiedInput = new nestedWindow.Request(toProxyHttp(input.url, state.transportPath), input);
            }
          } catch (_) {}
          return OriginalFetch.call(this, proxiedInput, init);
        };
      }
      const xhrPrototype = nestedWindow.XMLHttpRequest && nestedWindow.XMLHttpRequest.prototype;
      const originalOpen = xhrPrototype && xhrPrototype.open;
      if (typeof originalOpen === 'function') {
        xhrPrototype.open = function(method, url, ...rest) {
          return originalOpen.call(this, method, toProxyHttp(url, state.transportPath), ...rest);
        };
      }
      const OriginalEventSource = nestedWindow.EventSource;
      if (typeof OriginalEventSource === 'function') {
        nestedWindow.EventSource = function(url, config) {
          return new OriginalEventSource(toProxyHttp(url, state.transportPath), config);
        };
        nestedWindow.EventSource.prototype = OriginalEventSource.prototype;
      }
      Object.defineProperty(nestedWindow, '__HERMES_BROWSER_WORKBENCH_NESTED_RUNTIME__', {
        configurable: true,
        value: true,
      });
    } catch (_) {}
  };
  const installNestedFrameDocumentBoundary = (nestedDocument, state) => {
    try {
      const nestedWindow = nestedDocument && nestedDocument.defaultView;
      if (!nestedWindow || nestedDocument.__HERMES_BROWSER_WORKBENCH_NESTED_BOUNDARY__) return false;
      const nodePrototype = nestedWindow.Node && nestedWindow.Node.prototype;
      const elementPrototype = nestedWindow.Element && nestedWindow.Element.prototype;
      if (!nodePrototype || !elementPrototype) return false;
      state.originalSetAttribute = elementPrototype.setAttribute;
      const process = (node) => rewriteNestedFrameNode(node, state);
      const installChildren = (node) => {
        try {
          if (!node) return;
          if (node.nodeType === 1 && isSameOriginBlankFrame(node)) installSameOriginBlankFrame(node);
          if (typeof node.querySelectorAll === 'function') node.querySelectorAll('iframe').forEach(installSameOriginBlankFrame);
        } catch (_) {}
      };
      if (typeof state.originalSetAttribute === 'function') {
        elementPrototype.setAttribute = function(name, value) {
          const attribute = String(name || '').toLowerCase();
          const nextValue = nestedFrameResourceAttribute(this, attribute)
            ? nestedFrameSetAttribute(this, attribute, value, state)
            : value;
          const result = state.originalSetAttribute.call(this, name, nextValue);
          if (attribute === 'src' || attribute === 'sandbox') installChildren(this);
          return result;
        };
      }
      const patchResourceProperty = (constructorName, property, attribute = property) => {
        try {
          const constructor = nestedWindow[constructorName];
      ...
      [
        ...
        ['HTMLEmbedElement', 'src'],
        ['HTMLObjectElement', 'data'],
      ].forEach(([constructorName, property]) => {
        patchResourceProperty(constructorName, property);
      });
      const textContentDescriptor = Object.getOwnPropertyDescriptor(nodePrototype, 'textContent');
      if (textContentDescriptor && typeof textContentDescriptor.set === 'function') {
        Object.defineProperty(nodePrototype, 'textContent', {
          ...
          set(value) {
            const nextValue = String(this && this.localName || '').toLowerCase() === 'style'
              ? proxyOpaqueCssText(value, state.transportPath)
              : value;
            return textContentDescriptor.set.call(this, nextValue);
          },
        });
      }
      const stylePrototype = nestedWindow.CSSStyleDeclaration && nestedWindow.CSSStyleDeclaration.prototype;
      if (stylePrototype) {
        const originalSetProperty = stylePrototype.setProperty;
        if (typeof originalSetProperty === 'function') {
          stylePrototype.setProperty = function(name, value, priority) {
        ...
        const patchCssProperty = (property) => {
          try {
            ...
            Object.defineProperty(stylePrototype, property, {
              ...
              get: descriptor.get,
              set(value) {
                return descriptor.set.call(this, proxyOpaqueCssText(value, state.transportPath));
              },
            });
          } catch (_) {}
        };
        [
          'cssText', 'background', 'backgroundImage', 'borderImage',
          'borderImageSource', 'content', 'cursor', 'filter', 'listStyle',
          'listStyleImage', 'mask', 'maskImage', 'webkitMask', 'webkitMaskImage',
        ].forEach(patchCssProperty);
      }
      const wrapInsertion = (prototype, method, multiple) => {
        const original = prototype && prototype[method];
        if (typeof original !== 'function') return;
        prototype[method] = function(...args) {
          const nodes = multiple ? args : args.slice(0, 1);
          nodes.forEach(process);
          const result = original.apply(this, args);
          nodes.forEach(installChildren);
          return result;
        };
      };
      ['appendChild','insertBefore','replaceChild'].forEach((method) => {
        wrapInsertion(nodePrototype, method, false);
      });
      ['append','prepend','replaceChildren'].forEach((method) => {
        wrapInsertion(elementPrototype, method, true);
      });
      installNestedFrameNetworkBoundary(nestedWindow, state);
      Object.defineProperty(nestedDocument, '__HERMES_BROWSER_WORKBENCH_NESTED_BOUNDARY__', {
        configurable: true,
        value: true,
      });
      rewriteNestedFrameNode(nestedDocument.documentElement, state);
      return true;
    } catch (_) {
      return false;
    }
  };
  const installSameOriginBlankFrame = (candidate) => {
    try {
      if (!isSameOriginBlankFrame(candidate) || nestedFrameStates.has(candidate)) return false;
      const nestedDocument = candidate.contentDocument;
      if (!nestedDocument) return false;
      const nestedId = `${frameId || 'frame'}-nested-${++nestedFrameSequence}`;
      const state = {transportPath: proxyTransportForFrame(nestedId), originalSetAttribute: null};
      nestedFrameStates.set(candidate, state);
      return installNestedFrameDocumentBoundary(nestedDocument, state);
    } catch (_) {
      return false;
    }
  };
  const installNestedFrameCreationBoundary = () => {
    try {
      const nodePrototype = window.Node && window.Node.prototype;
      const elementPrototype = window.Element && window.Element.prototype;
      if (!nodePrototype || !elementPrototype || nodePrototype.__HERMES_NESTED_FRAME_CREATION_BOUNDARY__) return false;
      const installTree = (node) => {
        try {
          if (!node) return;
          if (node.nodeType === 1 && isSameOriginBlankFrame(node)) installSameOriginBlankFrame(node);
          if (typeof node.querySelectorAll === 'function') node.querySelectorAll('iframe').forEach(installSameOriginBlankFrame);
      ...
      // Detached iframe elements do not expose contentDocument yet. Wrap the
      // current insertion methods (which may already contain the opaque-frame
      // boundary) and install immediately after attachment, before caller code
      // can clone styles/scripts into the new document in the same task.
      const wrapInsertion = (prototype, method, multiple) => {
        const original = prototype && prototype[method];
        if (typeof original !== 'function') return;
        prototype[method] = function(...args) {
          const result = original.apply(this, args);
          (multiple ? args : args.slice(0, 1)).forEach(installTree);
          return result;
        };
      };
      ['appendChild','insertBefore','replaceChild'].forEach((method) => {
      ...
      Object.defineProperty(nodePrototype, '__HERMES_NESTED_FRAME_CREATION_BOUNDARY__', {
        configurable: true,
        value: true,
      });
      return true;
  ...
  try {
    installNestedFrameCreationBoundary();
    document.querySelectorAll('iframe').forEach(installSameOriginBlankFrame);
    if (window.MutationObserver && document.documentElement) {
      const nestedFrameObserver = new MutationObserver((records) => {
        records.forEach((record) => {
          if (record.addedNodes) record.addedNodes.forEach((node) => {
            if (node.nodeType === 1 && isSameOriginBlankFrame(node)) installSameOriginBlankFrame(node);
            if (node.querySelectorAll) node.querySelectorAll('iframe').forEach(installSameOriginBlankFrame);
          });
        });
      });
      nestedFrameObserver.observe(document.documentElement, {subtree:true, childList:true});
    }
  } catch (_) {}
  let proxyNavigationRedirecting = false;
  let rscNavigationGeneration = 0;
  ...
  const rscDocumentNavigationTarget = (input, init) => {
    try {
      if (methodOf(input, init, 'GET') !== 'GET') return '';
      const headers = new Headers();
      const appendHeaders = (value) => {
        if (!value) return;
        new Headers(value).forEach((headerValue, headerName) => headers.set(headerName, headerValue));
      };
      appendHeaders(input && input.headers);
      appendHeaders(init && init.headers);
      if (String(headers.get('rsc') || '') !== '1') return '';
      if (
        headers.has('next-router-prefetch')
        || headers.has('next-router-segment-prefetch')
        || String(headers.get('purpose') || '').toLowerCase() === 'prefetch'
        || String(headers.get('sec-purpose') || '').toLowerCase() === 'prefetch'
      ) return '';
      const rawUrl = requestUrlOf(input);
      if (!rawUrl) return '';
      const requested = new URL(rawUrl, location.href);
      let intended = requested;
      if (requested.origin === location.origin) {
        intended = new URL(`${requested.pathname}${requested.search}${requested.hash}`, targetOrigin);
      }
      if (intended.origin !== targetOrigin || intended.username || intended.password) return '';
      intended.searchParams.delete('_rsc');
      return intended.href;
    } catch (_) {
      return '';
    }
  };
  const rscDocumentNavigationCommitTarget = (
    intendedTarget,
    generation,
    latestGeneration,
    currentUrl,
    ...
    try {
      if (
        ...
        || !response
        || !response.ok
        || !String(response.headers && response.headers.get('content-type') || '')
          .toLowerCase()
          .startsWith('text/x-component')
      ) return '';
      const intended = new URL(intendedTarget);
      const current = new URL(currentUrl);
      intended.searchParams.delete('_rsc');
      current.searchParams.delete('_rsc');
      return intended.href === current.href ? '' : intended.href;
    } catch (_) {
      return '';
    }
  };
  ...
  if (typeof OriginalFetch === 'function') {
    window.fetch = function(input, init) {
      ...
      const rscNavigationTarget = rscDocumentNavigationTarget(input, init);
      const rscNavigationGenerationAtStart = rscNavigationTarget
        ? ++rscNavigationGeneration
        : 0;
      ...
      return OriginalFetch.call(this, proxiedInput, init).then((response) => {
        ...
        const rscNavigationCommitTarget = rscDocumentNavigationCommitTarget(
          rscNavigationTarget,
          rscNavigationGenerationAtStart,
          rscNavigationGeneration,
          currentTargetUrl(),
          response,
          proxyNavigationRedirecting
        );
        if (rscNavigationCommitTarget) {
          const proxiedNavigation = toProxyHttp(rscNavigationCommitTarget);
          if (proxiedNavigation) {
            proxyNavigationRedirecting = true;
            location.assign(proxiedNavigation);
          }
        }
        return response;
      }, (error) => {
  ...
  const browserWorkbenchNewTabUrl = (candidate) => {
    try {
      const raw = String(candidate === undefined || candidate === null ? '' : candidate).trim();
      if (!raw || raw.length > 4096) return '';
      const resolved = targetUrlFromProxyHref(raw) || new URL(raw, currentTargetUrl() || targetUrl).href;
      const parsed = new URL(resolved);
      if ((parsed.protocol !== 'http:' && parsed.protocol !== 'https:') || parsed.username || parsed.password) return '';
      return parsed.href;
    } catch (_) { return ''; }
  };
  const requestBrowserWorkbenchNewTab = (candidate) => {
    const url = browserWorkbenchNewTabUrl(candidate);
    if (!url) return false;
    post({type:'open-tab', url});
    return true;
  };
  const interceptBrowserWorkbenchNewTab = (event, candidate) => {
    try { if (event && typeof event.preventDefault === 'function') event.preventDefault(); } catch (_) {}
    requestBrowserWorkbenchNewTab(candidate);
    // Invalid/unsupported destinations still remain inside the sandbox instead
    // of falling through to a native host-browser popup.
    return true;
  };
  const nativeWindowOpen = typeof window.open === 'function' ? window.open : null;
  try {
    window.open = function(url, target, features) {
      const targetName = String(target || '_blank').trim().toLowerCase();
      if (targetName === '_blank') {
        requestBrowserWorkbenchNewTab(url);
        return null;
      }
      return nativeWindowOpen ? nativeWindowOpen.apply(this, arguments) : null;
    };
  } catch (_) {}
  ...
  document.addEventListener('click', (event) => {
    ...
    const targetName = String(anchor.target || anchor.getAttribute && anchor.getAttribute('target') || '').trim().toLowerCase();
    if (targetName === '_blank' || event.button === 1 || event.metaKey || event.ctrlKey) {
      const href = anchor.getAttribute && anchor.getAttribute('href');
      interceptBrowserWorkbenchNewTab(event, href || anchor.href);
      return;
    }
    if (event.button > 0 || event.shiftKey || event.altKey || (targetName && targetName !== '_self')) return;
...
def _browser_proxy_rewrite_body(
    ...
    *,
    ...
    if "text/css" in ctype or target_url.lower().split("?", 1)[0].endswith(".css"):
        text = data.decode("utf-8", errors="replace")
        # External stylesheets are not part of the target framework's hydrated
        # HTML attribute contract. Canonicalize every nested CSS asset while the
        # session/frame owner is still known: a recovered clean-root stylesheet
        # has a clean WebUI URL, so its font/image requests otherwise carry that
        # stylesheet as their referrer and can no longer be recovered back to the
        # target session.
...
def _browser_proxy_send_error(handler, target_url: str, message: str, *, status: int, method: str = "GET") -> bool:
    ...
    handler.send_header("Cache-Control", "no-store")
    handler.send_header("Referrer-Policy", _BROWSER_PROXY_REFERRER_POLICY)
    handler.send_header(
        "Content-Security-Policy",
...
def handle_browser_workbench_proxy_request(handler, parsed, *, method: str = "GET") -> bool:
    ...
    try:
        with proxy_state.open(
            request,
            timeout=_browser_proxy_timeout_for_target(target_url),
        ) as response:
...
def _extract_session_action(path: str) -> tuple[str, str] | None:
    ...
    if not session_id or "/" in session_id or not session_id.startswith(_SESSION_PREFIX):
        return None
    if action not in _BROWSER_WORKBENCH_POST_ACTIONS:
        return None
...
_NO_BROWSER_WORKBENCH_REQUEST_BODY = object()


def _call_browser_workbench_session_backend(
    session_id: str,
    method_name: str,
    body=_NO_BROWSER_WORKBENCH_REQUEST_BODY,
) -> tuple[dict, int]:
    """Run one backend session operation and sanitize its public payload."""
    backend = _backend_for_browser_workbench_session(session_id)
    method = getattr(backend, method_name)
    if body is _NO_BROWSER_WORKBENCH_REQUEST_BODY:
        payload, status = method(session_id)
    else:
        payload, status = method(session_id, body)
...
def interact_browser_workbench_session(session_id: str, body: dict | None = None) -> tuple[dict, int]:
    return _call_browser_workbench_session_backend(session_id, "interact", body)
...
def _hard_reload_browser_workbench_session(session_id: str, body: dict | None = None) -> tuple[dict, int]:
    # Preserve the existing best-effort contract: the cache-clear response is
    # ignored and the subsequent reload determines the public response.
    clear_browser_workbench_cache(session_id)
    return reload_browser_workbench_session(session_id, body)


_BROWSER_WORKBENCH_POST_ACTIONS = {
    "navigate": (navigate_browser_workbench_session, True),
    "reload": (reload_browser_workbench_session, True),
    "hard-reload": (_hard_reload_browser_workbench_session, True),
    "stop-loading": (stop_loading_browser_workbench_session, True),
    "back": (back_browser_workbench_session, True),
    "forward": (forward_browser_workbench_session, True),
    "clear-history": (clear_browser_workbench_history, False),
    "clear-cookies": (clear_browser_workbench_cookies, False),
    "clear-cache": (clear_browser_workbench_cache, False),
    "devtools": (open_browser_workbench_devtools, True),
    "frame": (frame_browser_workbench_session, True),
    "screenshot": (capture_browser_workbench_screenshot, True),
    "inspect": (inspect_browser_workbench_session, True),
    "interact": (interact_browser_workbench_session, True),
}
...
def handle_browser_workbench_post(handler, parsed, body: dict | None = None) -> bool:
    ...
    if session_action:
        ...
        action_handler, accepts_body = _BROWSER_WORKBENCH_POST_ACTIONS[action]
        if accepts_body:
            payload, status = action_handler(session_id, body)
        else:
            payload, status = action_handler(session_id)
        j(handler, payload, status=status)
        return True
