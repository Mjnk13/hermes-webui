...
class _LiveToolOutputCoalescer:
    ...
    Sending one journal record and browser event per line starves the browser
...
def _session_payload_with_full_messages(session, *, tool_calls=None):
    ...
    # Provider replay metadata and browser transcript content have different
    # owners. Codex can persist visible commentary only inside
    # codex_message_items while top-level content stays empty for exact replay;
    # project it into copies for every terminal browser payload (success,
    # cancellation, and error) without altering session/model context.
    display_messages = messages
    try:
        from api.routes import _messages_for_display_payload

        display_messages = _messages_for_display_payload(messages)
    except Exception:
        logger.debug('Failed to materialize terminal display messages', exc_info=True)
    raw = session.compact() | {
...
def _session_payload_with_terminal_window(session, *, tool_calls=None):
    ...
    embedding its entire transcript in the terminal event only makes the browser
    ...
    try:
        ...
        if offset > 0:
            ...
            window = _messages_for_display_payload(window)
...
_AGENTPET_NOTIFICATIONS_ENV = "HERMES_WEBUI_AGENTPET_NOTIFICATIONS"
...
def _agentpet_notifications_enabled(environ: dict[str, str] | None = None) -> bool:
    """Return false for pytest workers unless a focused bridge test opts in."""
    source = os.environ if environ is None else environ
    configured = str(source.get(_AGENTPET_NOTIFICATIONS_ENV) or "").strip().lower()
    if configured:
        return configured in {"1", "true", "yes", "on"}
    return not bool(source.get("PYTEST_CURRENT_TEST"))
...
def _notify_agentpet(
    ...
    *,
    ...
    if not _agentpet_notifications_enabled() or event_name not in _AGENTPET_STATE_EVENTS or not session_id:
        return
...
def _start_agentpet_heartbeat(
    *,
    ...
    if not _agentpet_notifications_enabled() or not session_id or not _agentpet_binary():
        return None, None
...
class _TurnOutcomeState:
    ...
    def _has_usable_output_locked(self) -> bool:
        return bool(
    ...
    def has_usable_output(self) -> bool:
        with self._lock:
            return self._has_usable_output_locked()
    ...
    def payload(self) -> dict:
        with self._lock:
            return {
                ...
                'usable_output': self._has_usable_output_locked(),
...
def _settle_result_messages(
    ...
    *,
    ...
    session.messages = _merge_display_messages_after_agent_result(
        ...
        current_browser_context_parts=current_browser_context_parts,
        current_client_message_id=current_client_message_id,
        source=source,
...
def _reasoning_only_replay_identity(msg):
    """Return a durable identity for a display-only reasoning row.

    Reasoning-only rows are intentionally absent from the API-safe context, so
    ``_restore_display_reasoning_metadata`` may project them back into an Agent
    result before the display merge.  They still need an identity at that merge
    boundary; otherwise an old row restored after a compacted result is treated
    as a new tail row on every turn and grows exponentially.

    Prefer the session-local stable message id, but include the reasoning
    payload because legacy alignment bugs could copy one id onto several
    distinct incomplete-reasoning snapshots.  Legacy rows without an id are
    only deduplicated when they carry a timestamp.  A timestamp-less/id-less
    row stays unkeyed rather than risking collapse of two legitimate turns with
    identical thinking text.
    """
    if not _is_reasoning_only_assistant_message(msg) or msg.get('_partial'):
        return None
    payload = {
        'content': msg.get('content'),
    ...
    if message_id is not None and not isinstance(message_id, bool):
        return (
            'reasoning_only_message_id',
            type(message_id).__name__,
            str(message_id),
            payload_key,
        )
    platform_message_id = msg.get('platform_message_id')
    if platform_message_id not in (None, ''):
        return ('reasoning_only_platform_id', str(platform_message_id), payload_key)
    timestamp = msg.get('timestamp')
    if timestamp in (None, ''):
        timestamp = msg.get('_ts')
    if timestamp in (None, ''):
        return None
    return (
        'reasoning_only_legacy',
        str(timestamp),
        payload_key,
    )
...
def _restore_display_reasoning_metadata(previous_messages, updated_messages):
    ...
    reasoning_first_index = {}
    for prev_idx, prev_msg in enumerate(previous_messages):
        replay_key = _reasoning_only_replay_identity(prev_msg)
        if replay_key is not None:
            reasoning_first_index.setdefault(replay_key, prev_idx)
    ...
    for prev_idx, prev_msg in enumerate(previous_messages):
        ...
        if (
            ...
            and reasoning_first_index.get(replay_key) != prev_idx
        ):
            continue
...
def _merge_display_messages_after_agent_result(
    ...
    # Reasoning-only replays belong at their first historical position. Keep
    # that canonical copy before running the existing reverse scan for partials
    # (which intentionally keeps the latest cancellation snapshot).
    _reasoning_seen = set()
    _reasoning_deduped = []
    for m in previous_display:
        reasoning_key = _reasoning_only_replay_identity(m)
        if reasoning_key is not None:
            if reasoning_key in _reasoning_seen:
                continue
            _reasoning_seen.add(reasoning_key)
        _reasoning_deduped.append(m)
    previous_display = _reasoning_deduped
    ...
    seen_reasoning_only = {
        ...
        if (replay_key := _reasoning_only_replay_identity(message)) is not None
    }
    current_user_key = _message_identity({'role': 'user', 'content': msg_text})
    current_user_in_candidates = any(
        _message_identity(m) == current_user_key or _looks_like_current_user_turn(m, msg_text)
    ...
    if _active_turn_identity:
        current_user_already_checkpointed = _active_turn_has_checkpoint(
            merged, _active_turn_identity
        )
    elif current_client_message_id:
    ...
    for msg in candidates:
        ...
        reasoning_replay_key = _reasoning_only_replay_identity(msg)
        if (
            reasoning_replay_key is not None
            and reasoning_replay_key in seen_reasoning_only
        ):
            continue
        ...
        if key is not None:
            seen.add(key)
        if reasoning_replay_key is not None:
            seen_reasoning_only.add(reasoning_replay_key)
    return merged
...
def _run_agent_streaming(
    ...
    *,
    ...
    def _prepare_turn_outcome_event(event, data):
        """Apply custom turn-outcome semantics before journaling an SSE frame."""
        # If cancelled, retain only established terminal signals. ``apperror``
        # is the canonical SSE failure frame and must reach relay close loops;
        # ``error`` remains accepted for legacy/adapter compatibility.
        if cancel_event.is_set() and not _success_writeback_committed and event not in ('cancel', 'apperror') and event != 'error':
            return None
    ...
    def put(event, data):
        prepared = _prepare_turn_outcome_event(event, data)
        if prepared is None:
            return
        event, data = prepared
        event_id = None
