...
def test_request_renderer_uses_real_chromium_when_native_devtools_are_configured():
    js = _read("static/browser_workbench.js")
    preferred = _js_function(js, "browserWorkbenchPreferredClientRenderer")
    program = "\n".join(
        [
            preferred,
            "const workbenchCapabilities={native_devtools:true,interactive_viewport:true,iframe_bridge:true};",
            "const native=browserWorkbenchPreferredClientRenderer();",
            "workbenchCapabilities.native_devtools=false;",
            "const fallback=browserWorkbenchPreferredClientRenderer();",
            "console.log(JSON.stringify({native,fallback}));",
        ]
    )

    assert _run_node_json(program) == {
        "native": "chromium-stream",
        "fallback": "iframe-bridge",
    }


def test_close_tab_is_optimistic_and_force_click_is_idempotent():
    js = _read("static/browser_workbench.js")
    close_tab = "async " + _js_function(js, "closeBrowserWorkbenchTab")
    program = "\n".join(
        [
            "const events=[];const requests=[];const resolvers=[];",
            "const parent={removeChild:node=>{node.parentNode=null;events.push(`removed:${node.id}`);}};",
            "const tabA={id:'browser-a',sessionId:'session-a',tabEl:{id:'tab-a',parentNode:parent}};",
            "const tabB={id:'browser-b',sessionId:'session-b',tabEl:{id:'tab-b',parentNode:parent}};",
            "const workbenchTabs=new Map([[tabA.id,tabA],[tabB.id,tabB]]);",
            "let activeBrowserWorkbenchTabId=tabA.id;let selectionModeTabId='';let areaCaptureMode=false;",
            "const wireDom=()=>{};const tabById=id=>workbenchTabs.get(id)||null;",
            "const getActiveWorkbenchTab=()=>workbenchTabs.get(activeBrowserWorkbenchTabId)||null;",
            "const setTabState=()=>{};const setStatus=()=>({});",
            "const sessionStatusUrl=id=>`/session/${id}`;",
            "const requestJSON=(url,options)=>{requests.push({url,method:options.method});return new Promise(resolve=>resolvers.push(resolve));};",
            "const isBrowserWorkbenchSessionMissingError=()=>false;const browserWorkbenchResolveStatus=()=>{};",
            "const browserWorkbenchClearStatus=()=>{};const setBrowserWorkbenchSelectionMode=()=>{};",
            "const cancelBrowserWorkbenchAreaCapture=()=>{};const clearBrowserWorkbenchLoadTimers=()=>{};",
            "const clearBrowserWorkbenchPendingHistoryTraversal=()=>{};const removeBrowserWorkbenchStoredSurface=()=>{};",
            "const renderActiveBrowserWorkbenchView=()=>events.push('render');",
            "const persistBrowserWorkbenchTabs=()=>events.push('persist');",
            "const activateBrowserWorkbenchTab=id=>{activeBrowserWorkbenchTabId=id;events.push(`activate:${id}`);};",
            "const switchPanel=panel=>{events.push(`panel:${panel}`);return Promise.resolve();};",
            "const showToast=message=>events.push(`toast:${message}`);",
            close_tab,
            "(async()=>{",
            "const first=closeBrowserWorkbenchTab(tabA.id);",
            "const immediate={hasA:workbenchTabs.has(tabA.id),hasB:workbenchTabs.has(tabB.id),active:activeBrowserWorkbenchTabId,requests:requests.length,removed:events.includes('removed:tab-a')};",
            "const second=closeBrowserWorkbenchTab(tabA.id);",
            "const forced={hasB:workbenchTabs.has(tabB.id),active:activeBrowserWorkbenchTabId,requests:requests.length};",
            "resolvers.forEach(resolve=>resolve({ok:true,status:'closed'}));",
            "const results=await Promise.all([first,second]);",
            "console.log(JSON.stringify({immediate,forced,results,finalTabs:Array.from(workbenchTabs.keys())}));",
...
def test_returning_to_loaded_tab_removes_restored_placeholder_from_other_tab():
    js = _read("static/browser_workbench.js")
    program = "\n".join(
        [
            "class FakeNode {",
            "  constructor(tag='div'){this.tagName=tag.toUpperCase();this.nodeType=1;this.parentNode=null;this.childNodes=[];this.dataset={};this.hidden=false;this.attributes={};this.listeners={};this.className='';this.textContent='';this.classList={add:(name)=>{this.className+=(this.className?' ':'')+name;},remove:()=>{}};this.style={removeProperty:()=>{}};}",
            "  appendChild(node){if(node.parentNode)node.parentNode.removeChild(node);this.childNodes.push(node);node.parentNode=this;return node;}",
            "  removeChild(node){const index=this.childNodes.indexOf(node);if(index!==-1)this.childNodes.splice(index,1);node.parentNode=null;return node;}",
            "  setAttribute(name,value){this.attributes[name]=String(value);}",
            "  addEventListener(type,callback){(this.listeners[type]||(this.listeners[type]=[])).push(callback);}",
            "  matches(selector){return selector.startsWith('.browser-workbench-frame-wrap[data-browser-workbench-tab-id')&&this.className.includes('browser-workbench-frame-wrap')&&!!this.dataset.browserWorkbenchTabId;}",
            "  querySelector(selector){return this.childNodes.find((node)=>node.matches&&node.matches(selector))||null;}",
            "  querySelectorAll(selector){return this.childNodes.filter((node)=>node.matches&&node.matches(selector));}",
            "}",
            "const CSS={escape:(value)=>String(value)};",
            "const document={createElement:(tag)=>new FakeNode(tag)};",
            "const host=new FakeNode('main');const viewportEl=host;",
            "let browserWorkbenchOverlayPreview=null;",
            "const tabA={id:'browser-tab-a',url:'https://example.test/a',bridgeUrl:'/bridge/a',renderer:'iframe-bridge',loadStatus:'success',contentState:'loaded',surfaceNode:null,surfaceUrl:''};",
            "const tabB={id:'browser-tab-b',url:'https://example.test/b',requestedUrl:'https://example.test/b',contentState:'restored'};",
            "const tabs=new Map([[tabA.id,tabA],[tabB.id,tabB]]);",
            "const tabById=(id)=>tabs.get(id)||null;const wireDom=()=>{};",
            "const stopBrowserWorkbenchChromiumStream=()=>{};const applyBrowserWorkbenchSurfaceZoom=()=>{};",
            "const browserWorkbenchDisplayLabel=()=>\"Browser\";const syncBrowserWorkbenchIframeLoadedLocation=()=>{};",
            "const syncBrowserWorkbenchIframeSelectionMode=()=>{};const setBrowserWorkbenchLoadStatus=()=>{};",
            ...
            _js_function(js, "browserWorkbenchContentPlaceholder"),
            _js_function(js, "stashBrowserWorkbenchSurface"),
            _js_function(js, "removeBrowserWorkbenchStoredSurface"),
            _js_function(js, "clearBrowserWorkbenchHostForRender"),
            _js_function(js, "setViewportMessage"),
            _js_function(js, "renderBrowserWorkbenchFrame"),
            "renderBrowserWorkbenchFrame(tabA,host);",
            "const originalSurface=tabA.surfaceNode;",
            "setViewportMessage(browserWorkbenchContentPlaceholder(tabB));",
            "const duringSwitch=host.childNodes.map(node=>node.className);",
            "renderBrowserWorkbenchFrame(tabA,host);",
            "const placeholders=host.childNodes.filter(node=>node.className==='browser-workbench-placeholder').map(node=>node.textContent);",
            "console.log(JSON.stringify({sameSurface:tabA.surfaceNode===originalSurface,contentState:tabA.contentState,duringSwitch,afterReturn:host.childNodes.map(node=>node.className),placeholders}));",
        ]
    )
    ...
    assert _run_node_json(program) == {
        ...
        "contentState": "loaded",
        "duringSwitch": [
            "browser-workbench-frame-wrap browser-workbench-frame-wrap--bridge",
            "browser-workbench-placeholder",
        ],
        "afterReturn": [
            "browser-workbench-frame-wrap browser-workbench-frame-wrap--bridge"
        ],
