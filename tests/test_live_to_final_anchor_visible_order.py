...
def test_anchor_scene_projection_tracks_active_mode():
    ...
    paint_live = _function_body(MESSAGES_JS, "_paintAnchorLiveScene") if "function _paintAnchorLiveScene" in MESSAGES_JS else render_live
    project_live = _function_body(MESSAGES_JS, "_projectLiveAnchorActivityScene")

    assert "_anchorSceneActiveMode()" in paint_live
    assert "_anchorSceneActiveMode()" in project_live
    assert "mode:'compact_worklog'" not in paint_live
