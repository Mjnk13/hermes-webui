...
def test_browser_surface_is_snapshot_backed_before_scoped_visibility_suppression():
    ...
    assert "borderRadius," in WORKBENCH
    assert "record.view.setBorderRadius(borderRadius)" in DESKTOP
    assert "wc.setBackgroundThrottling(false)" in DESKTOP
    assert "wc.setBackgroundThrottling(true)" in DESKTOP
...
def test_desktop_runtime_supports_native_webcontents_view_border_radius():
    electron_spec = DESKTOP_PACKAGE["devDependencies"]["electron"]
    electron_major = int(electron_spec.lstrip("^~<>= ").split(".", 1)[0])
    assert electron_major >= 33


@pytest.mark.skipif(NODE is None, reason="node not on PATH")
def test_live_native_surface_uses_the_same_rounded_content_box_as_snapshot():
    payload_function = _function(WORKBENCH, "currentNativeBoundsPayload")
    script = f"""
let viewportEl={{getBoundingClientRect:()=>({{left:20,top:30,width:502,height:302}})}};
const wireDom=()=>{{}};
const document={{querySelector:()=>({{}})}};
const window={{
  devicePixelRatio:2,
  getComputedStyle:()=>({{
    borderLeftWidth:'1px',borderTopWidth:'1px',
...
const browserWorkbenchOverlaySuppression=bounds=>({{suppressed:false,overlayCount:0,generation:1,bounds}});
{payload_function}
const payload=currentNativeBoundsPayload({{
  id:'tab-1',sessionId:'session-1',renderer:'electron-native',zoom:1.25,
}},true);
...
"""
    ...
    assert payload["bounds"] == {"x": 21, "y": 31, "width": 500, "height": 300}
    assert payload["borderRadius"] == 14
    assert payload["visible"] is True


@pytest.mark.skipif(NODE is None, reason="node not on PATH")
def test_main_process_clips_live_native_view_without_overlay_lifecycle_changes():
    set_bounds = _function(DESKTOP, "setNativeBounds")
    script = f"""
const calls=[];
const record={{
  id:'session-1',visible:false,zoom:1,viewport:{{}},selectionMode:false,
  view:{{
    setBounds:value=>calls.push(['setBounds',value]),
    setBorderRadius:value=>calls.push(['setBorderRadius',value]),
    setVisible:value=>calls.push(['setVisible',value]),
    webContents:{{setZoomFactor:value=>calls.push(['setZoomFactor',value])}},
  }},
}};
...
const applicationOverlaySuppression={{suppressed:false,generation:0,overlayCount:0,sessionId:''}};
const hideRecord=()=>{{}};
const addNativeViewToWindow=()=>calls.push(['attach']);
const applicationOverlaySuppressionAppliesToRecord=()=>false;
const setNativeSelectionMode=()=>{{}};
{set_bounds}
const result=setNativeBounds({{
  sessionId:'session-1',tabId:'tab-1',visible:true,zoom:1.25,
  bounds:{{x:21,y:31,width:500,height:300}},borderRadius:14,devicePixelRatio:2,
}});
...
"""
    ...
    assert result.returncode == 0, result.stderr
    data = json.loads(result.stdout)
    assert data["result"]["border_radius"] == 14
    assert data["calls"][:3] == [
        ["attach"],
        ["setBounds", {"x": 21, "y": 31, "width": 500, "height": 300}],
        ["setBorderRadius", 14],
    ]
    assert all(call[0] not in {"loadURL", "reload", "close"} for call in data["calls"])
...
const throttling=[];
let focusRestores=0;
const webContents={{isFocused:()=>true,isDestroyed:()=>false,focus:()=>{{focusRestores+=1;}},setBackgroundThrottling:value=>throttling.push(value)}};
...
const record={{id:'session-1',view,visible:true,focusBeforeApplicationOverlay:false,applicationOverlayThrottleRestoreToken:0}};
const tabs=new Map([['session-1',record]]);
...
process.stdout.write(JSON.stringify({{visibility,throttling,focusRestores,opened,staleClose,oneStillOpen,closed}}));
"""
    result = subprocess.run([NODE, "-e", script], text=True, capture_output=True, timeout=30)
    assert result.returncode == 0, result.stderr
    data = json.loads(result.stdout)
    assert data["visibility"] == [False, True]
    assert data["throttling"] == [False, True]
...
@pytest.mark.skipif(NODE is None, reason="node not on PATH")
def test_rapid_overlay_reopen_cancels_stale_background_throttle_restore():
    functions = "\n".join(
        _function(DESKTOP, name)
        for name in (
            "applicationOverlaySuppressionAppliesToRecord",
...
const timers=[];
const webContents={{
  isFocused:()=>false,isDestroyed:()=>false,focus:()=>{{}},
  setBackgroundThrottling:value=>throttling.push(value),
}};
const view={{webContents,isDestroyed:()=>false,setVisible:value=>visibility.push(value)}};
const record={{
  id:'session-1',view,visible:true,focusBeforeApplicationOverlay:false,
...
{functions}
setApplicationOverlaySuppression({{suppressed:true,generation:1,overlayCount:1,sessionId:'session-1'}});
setApplicationOverlaySuppression({{suppressed:false,generation:2,overlayCount:0,sessionId:'session-1'}});
setApplicationOverlaySuppression({{suppressed:true,generation:3,overlayCount:1,sessionId:'session-1'}});
timers.forEach(callback=>callback());
process.stdout.write(JSON.stringify({{visibility,throttling,suppressed:applicationOverlaySuppression.suppressed}}));
"""
    result = subprocess.run([NODE, "-e", script], text=True, capture_output=True, timeout=30)
    assert result.returncode == 0, result.stderr
    data = json.loads(result.stdout)
