...
def test_live_tool_output_burst_is_coalesced_before_sse_delivery():
    ...
    assert len(tool_events) <= 20, (
        ...
        "the browser renderer event loop while the command is running"
    )
...
@pytest.mark.skipif(shutil.which("node") is None, reason="node required")
def test_reconnect_replay_batches_anchor_scene_paints_until_cursor_catches_up():
    """A hidden-tab replay must update state per event without repainting per event.

    Production replay 422..995 contained 574 events.  The old path rebuilt the
    complete accumulated activity scene on every tool/reasoning boundary, making
    return-to-tab work grow quadratically.  Exercise the real batching/cursor
    helpers with 150 replay boundaries: one cheap ownership paint plus one final
    catch-up paint is the bounded contract.
    """
    messages_source = (ROOT / "static" / "messages.js").read_text(encoding="utf-8")
    helpers = "\n".join(
        [
            _nested_function_source_from(messages_source, "_streamStatusJournalCursor"),
            _nested_function_source_from(messages_source, "_runJournalReplayAfterSeq"),
            _nested_function_source_from(messages_source, "_mergeAnchorReplayRenderOptions"),
            _nested_function_source_from(messages_source, "_paintAnchorLiveScene"),
            _nested_function_source_from(messages_source, "_flushAnchorReplayRender"),
            _nested_function_source_from(messages_source, "_scheduleAnchorReplayIdlePaint"),
            _nested_function_source_from(messages_source, "_prepareAnchorReplayRenderBatch"),
            _nested_function_source_from(messages_source, "_anchorReplayRenderBatchActive"),
            _nested_function_source_from(messages_source, "_anchorReplayHasVisibleSceneOwner"),
            _nested_function_source_from(messages_source, "_renderAnchorLiveScene"),
            _nested_function_source_from(messages_source, "_rememberRunJournalCursor"),
        ]
    )
    harness = textwrap.dedent(f"""
        const paints=[];
        const activeSid='session-replay';
        const streamId='stream-replay';
        const _anchorRegistry={{anchor:{{activity_events:[]}}}};
        const INFLIGHT={{[activeSid]:{{}}}};
        const window={{
          isLiveAnchorActivitySceneOwner:()=>paints.length>0,
          _renderLiveAnchorActivitySceneForStream:(_stream,_sid,options)=>{{
            paints.push({{...options}});
            return true;
          }},
        }};
        ...
        const _anchorSceneActiveMode=()=> 'compact_worklog';
        const _noteRunJournalActivity=()=>{{}};
        const _throttledPersist=()=>{{}};
        {helpers}

        const prepared=_prepareAnchorReplayRenderBatch({{journal:{{last_seq:150}}}});
        for(let seq=1;seq<=150;seq+=1){{
          _renderAnchorLiveScene({{animateMutationDiffs:seq%2===0}});
          _rememberRunJournalCursor({{lastEventId:`${{streamId}}:${{seq}}`}});
        }}
        const replayPaintCount=paints.length;
        const replayFinalPaint=paints[paints.length-1];
        _renderAnchorLiveScene({{animateMutationDiffs:false}});
        console.log(JSON.stringify({{
          prepared,
          replayPaintCount,
          paintCountAfterLiveEvent:paints.length,
          firstPaint:paints[0],
          replayFinalPaint,
          finalPaint:paints[paints.length-1],
          lastSeq:_lastRunJournalSeq,
          target:_anchorReplayRenderThroughSeq,
          pending:_anchorReplayRenderPending,
        }}));
    """)
    result = subprocess.run(
        ["node", "-e", harness],
    ...
    assert output == {
        "prepared": True,
        "replayPaintCount": 2,
        "paintCountAfterLiveEvent": 3,
        "firstPaint": {"mode": "compact_worklog", "animateMutationDiffs": False},
        "replayFinalPaint": {"mode": "compact_worklog", "animateMutationDiffs": True},
        "finalPaint": {"mode": "compact_worklog", "animateMutationDiffs": False},
        "lastSeq": 150,
        "target": 0,
        "pending": False,
    }
...
def test_every_replay_eventsource_is_prepared_with_its_status_cursor():
    messages_source = (ROOT / "static" / "messages.js").read_text(encoding="utf-8")
    ui_source = (ROOT / "static" / "ui.js").read_text(encoding="utf-8")
    compact = re.sub(r"\s+", "", messages_source)
    ui_compact = re.sub(r"\s+", "", ui_source)

    assert "_wireSSE(newEventSource" in compact
    assert compact.count("replayStatus:st") >= 3
    assert "replayStatus:status" in compact
    assert "_prepareAnchorReplayRenderBatch(wireOptions.replayStatus)" in compact
    assert "deferAnchorReplayRender:_anchorReplayShouldDeferLegacyToolCard()" in compact
    assert "if(opts.deferAnchorReplayRender===true)return;" in ui_compact


@pytest.mark.skipif(shutil.which("node") is None, reason="node required")
def test_tool_output_replay_defers_legacy_card_fallback_by_behavior():
    """A replayed output chunk without a mounted row must not build a legacy card."""
    ui_source = (ROOT / "static" / "ui.js").read_text(encoding="utf-8")
    append_source = _nested_function_source_from(ui_source, "appendLiveToolCard")
    harness = textwrap.dedent(f"""
        const S={{session:{{session_id:'session-replay'}},activeStreamId:'stream-replay'}};
        {append_source}
        appendLiveToolCard(
          {{tid:'tool-1',name:'terminal'}},
          {{
            sessionId:'session-replay',
            streamId:'stream-replay',
            deferAnchorReplayRender:true,
          }}
        );
...
def test_restored_anchor_scene_is_not_rebuilt_for_each_persisted_tool_call():
    ...
    transcript rebuilds and blocks the browser renderer for several seconds.
    """
    sessions_source = (ROOT / "static" / "sessions.js").read_text(encoding="utf-8")
