...
def test_async_delegation_completion_nudges_active_main_at_safe_boundary(monkeypatch):
    ...
    try:
        ...
        # Durable async-delivery state is now the sole persistence owner. The
        # custom handoff still nudges the active parent, but must not duplicate
        # the completion in the legacy process-local deferred queue.
