...
def test_usable_output_property_and_payload_stay_in_sync_for_every_signal():
    outcomes = [_TurnOutcomeState('stream-empty', 'session-empty')]

    text = _TurnOutcomeState('stream-text', 'session-text')
    text.mark_text('answer')
    outcomes.append(text)

    reasoning = _TurnOutcomeState('stream-reasoning', 'session-reasoning')
    reasoning.mark_reasoning('thought')
    outcomes.append(reasoning)

    tool = _TurnOutcomeState('stream-tool', 'session-tool')
    tool.mark_tool('call-1')
    outcomes.append(tool)

    for key in ('assistant_text', 'reasoning', 'tool_output'):
        result = _TurnOutcomeState(f'stream-result-{key}', f'session-result-{key}')
        result.observe_result({key: True})
        outcomes.append(result)

    assert [outcome.has_usable_output for outcome in outcomes] == [
        False,
        True,
        True,
        True,
    ...
    assert [outcome.payload()['usable_output'] for outcome in outcomes] == [
        outcome.has_usable_output for outcome in outcomes
    ]
