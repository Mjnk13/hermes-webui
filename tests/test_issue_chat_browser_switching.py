...
def test_electron_shell_and_visited_pages_use_isolated_backgrounds():
    assert "const SHELL_SURFACE_BACKGROUND = '#0D0D1A';" in DESKTOP_MAIN
    assert "const BROWSER_PAGE_BACKGROUND = '#FFFFFF';" in DESKTOP_MAIN
    assert "backgroundColor: SHELL_SURFACE_BACKGROUND" in DESKTOP_MAIN
    assert "mainWindow.setBackgroundColor(SHELL_SURFACE_BACKGROUND)" in DESKTOP_MAIN
    assert "mainWindow.webContents.setBackgroundColor(SHELL_SURFACE_BACKGROUND)" in DESKTOP_MAIN
    assert "view.setBackgroundColor(BROWSER_PAGE_BACKGROUND)" in DESKTOP_MAIN
    assert "view.webContents.setBackgroundColor(BROWSER_PAGE_BACKGROUND)" in DESKTOP_MAIN
    assert "view.setBackgroundColor(SHELL_SURFACE_BACKGROUND)" not in DESKTOP_MAIN
    assert "view.webContents.setBackgroundColor(SHELL_SURFACE_BACKGROUND)" not in DESKTOP_MAIN
