...
eval(extractFunc('_assistantMutationTimelineNode'));
...
def test_mutation_timeline_helper_preserves_live_and_settled_render_options():
    script = f"""
const fs=require('fs');
const src=fs.readFileSync({json.dumps(str(UI_JS))},'utf8');
{EXTRACT_FUNC_JS}
let mergeCalls=0;
global._mergeLiveAssistantModifiedItems=(items)=>{{
  mergeCalls+=1;
  return items.map(item=>({{...item,merged:true}}));
}};
global._assistantModifiedFilesNode=(items,classes,opts)=>({{items,classes,opts}});
eval(extractFunc('_assistantMutationTimelineNode'));
const live=_assistantMutationTimelineNode(
  [{{path:'live.js'}}],
  'assistant-worklog-modified-files-row',
  {{live:true,collapsed:false,simulateDiffStream:true}}
);
const settled=_assistantMutationTimelineNode(
  [{{path:'settled.js'}}],
  'assistant-anchor-modified-files-row',
  {{live:false,collapsed:true,simulateDiffStream:false}}
);
process.stdout.write(JSON.stringify({{mergeCalls,live,settled}}));
"""
    result = _run_node(script)
    ...
    assert result == {
        "mergeCalls": 1,
        "live": {
            "items": [{"path": "live.js", "merged": True}],
            "classes": "assistant-worklog-modified-files-row assistant-timeline-mutation-event",
            "opts": {"collapsed": False, "simulateDiffStream": True},
        },
        "settled": {
            "items": [{"path": "settled.js"}],
            "classes": "assistant-anchor-modified-files-row assistant-timeline-mutation-event",
            "opts": {"collapsed": True, "simulateDiffStream": False},
        },
    }
