...
def test_settled_assistant_modified_files_are_not_hidden_by_worklog_reason_mirroring():
    fold_block = _function_block(UI_JS, "_foldWorklogSourceAnchor")
    ...
    assert guard in fold_block
    assert "assistant-segment-worklog-source" in fold_block
    assert "_foldWorklogSourceAnchor(anchor)" in sync_block
    assert "_foldWorklogSourceAnchor(anchor)" in append_block
