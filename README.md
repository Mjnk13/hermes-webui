...
On macOS, when an installed Hermes WebUI launchd job is configured for the same port, `ctl.sh` uses that job as the single WebUI process owner. `stop` unloads it before terminating the server, `start` loads it again, and `restart` performs both steps in order.
