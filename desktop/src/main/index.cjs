...
// Keep the Hermes shell's paint fallback separate from visited page content.
// A website can intentionally leave html/body or individual surfaces
// transparent; backing a WebContentsView with the shell's dark color changes
// how those pixels and every semi-transparent color above them are rendered.
// Chrome uses an opaque white page fallback, so use the same neutral surface
// without forcing or emulating either prefers-color-scheme value.
const SHELL_SURFACE_BACKGROUND = '#0D0D1A';
const BROWSER_PAGE_BACKGROUND = '#FFFFFF';
...
function createMainWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    minHeight: 680,
    title: 'Hermes WebUI',
    backgroundColor: SHELL_SURFACE_BACKGROUND,
    webPreferences: createShellWebPreferences({
      preloadPath: path.join(__dirname, '../preload/index.js'),
      bridgePayload: payload,
    }),
  });

  try { mainWindow.setBackgroundColor(SHELL_SURFACE_BACKGROUND); } catch (_) {}
  try { mainWindow.webContents.setBackgroundColor(SHELL_SURFACE_BACKGROUND); } catch (_) {}
...
function sendBrowserFindMessage(channel, record, payload) {
...
function isBrowserFindShortcut(input) {
  const type = String(input && input.type || '');
  const key = String(input && input.key || '').toLowerCase();
  return type === 'keyDown' && key === 'f' && !!(input && (input.meta || input.control)) && !(input && input.alt);
}

function findInNativePage(payload) {
  ...
  if (!recordHasLiveWebContents(record)) throw new Error('No browser tab is active.');
  const query = String(payload && payload.query || '').slice(0, 4096);
  if (!query) {
    try { record.view.webContents.stopFindInPage('clearSelection'); } catch (_) {}
    record.findRequestId = 0;
    record.findQuery = '';
    sendBrowserFindMessage('browser-workbench:find-result', record, {
      request_id: 0,
      active_match_ordinal: 0,
      matches: 0,
      final_update: true,
    });
    return { ok: true, request_id: 0, session_id: record.id };
  }
  const requestId = record.view.webContents.findInPage(query, {
    forward: !(payload && payload.forward === false),
    // Electron names this option from Chromium's perspective: true begins a
    // new query, false advances within the existing result set.
    findNext: !!(payload && payload.findNext === true),
    matchCase: !!(payload && payload.matchCase === true),
  });
  record.findRequestId = requestId;
  record.findQuery = query;
  return { ok: true, request_id: requestId, session_id: record.id };
}

function stopFindInNativePage(payload) {
  const sessionId = String(payload && (payload.sessionId || payload.session_id) || activeSessionId || '').trim();
  const record = sessionId ? tabs.get(sessionId) : null;
  if (!recordHasLiveWebContents(record)) return { ok: true, status: 'inactive', session_id: sessionId };
  const action = String(payload && payload.action || 'clearSelection');
  const safeAction = action === 'keepSelection' || action === 'activateSelection' ? action : 'clearSelection';
  try { record.view.webContents.stopFindInPage(safeAction); } catch (_) {}
  record.findRequestId = 0;
  record.findQuery = '';
  if (payload && payload.focusPage === true && record.visible && activeSessionId === record.id) {
    try { record.view.webContents.focus(); } catch (_) {}
  }
  return { ok: true, status: 'stopped', session_id: record.id };
}
...
function ensureTab(payload) {
  ...
  // they do in Chrome and cannot reveal the dark application container.
  try { view.setBackgroundColor(BROWSER_PAGE_BACKGROUND); } catch (_) {}
  try { view.webContents.setBackgroundColor(BROWSER_PAGE_BACKGROUND); } catch (_) {}
  ...
  record = {
    ...
    applicationOverlayThrottleRestoreToken: 0,
    findRequestId: 0,
    findQuery: '',
    selectionMode: false,
    loadStatus: 'idle',
  ...
  view.webContents.on('before-input-event', (event, input) => {
    if (isBrowserFindShortcut(input) && record.visible && activeSessionId === record.id) {
      event.preventDefault();
      if (mainWindow && mainWindow.webContents && !mainWindow.webContents.isDestroyed()) {
        mainWindow.webContents.focus();
        sendBrowserFindMessage('browser-workbench:find-requested', record);
      }
      return;
    }
  ...
  view.webContents.on('found-in-page', (_event, result) => {
    if (!result || Number(result.requestId) !== Number(record.findRequestId)) return;
    sendBrowserFindMessage('browser-workbench:find-result', record, {
      request_id: Number(result.requestId) || 0,
      active_match_ordinal: Number(result.activeMatchOrdinal) || 0,
      matches: Number(result.matches) || 0,
      final_update: result.finalUpdate === true,
    });
  });
  ...
  view.webContents.on('did-start-navigation', (_event, url, isInPlace, isMainFrame) => {
    ...
    if (isMainFrame !== false && isInPlace !== true && isPublicBrowserNavigationUrl(url)) {
      if (record.findRequestId) stopFindInNativePage({ session_id: record.id });
...
function hideRecord(record) {
  ...
  record.applicationOverlayThrottleRestoreToken = (record.applicationOverlayThrottleRestoreToken || 0) + 1;
  try { record.view.webContents.setBackgroundThrottling(true); } catch (_) {}
...
function applyApplicationOverlaySuppressionToRecord(record, suppressed, wasSuppressed) {
  ...
  if (suppressed && !wasSuppressed) {
    // Keep the Chromium compositor and Page Visibility state active while the
    // native child view is visually replaced by its captured frame. Otherwise
    // setVisible(false/true) can background the renderer and expose a fresh
    // compositor frame when the overlay closes.
    record.applicationOverlayThrottleRestoreToken = (record.applicationOverlayThrottleRestoreToken || 0) + 1;
  ...
  if (!suppressed && wasSuppressed) {
    const throttleRestoreToken = (record.applicationOverlayThrottleRestoreToken || 0) + 1;
    record.applicationOverlayThrottleRestoreToken = throttleRestoreToken;
    ...
    setTimeout(() => {
      if ((record.applicationOverlayThrottleRestoreToken || 0) !== throttleRestoreToken) return;
      if (applicationOverlaySuppressionAppliesToRecord(applicationOverlaySuppression, record)) return;
      try { wc.setBackgroundThrottling(true); } catch (_) {}
    }, 80);
  }
}
...
function setNativeBounds(payload) {
  ...
  const borderRadius = Math.max(0, Math.min(
    Math.round(Number(payload.borderRadius ?? payload.border_radius) || 0),
    Math.floor(nextBounds.width / 2),
    Math.floor(nextBounds.height / 2),
  ));
  record.visible = true;
  ...
  if (typeof record.view.setBorderRadius !== 'function') {
    throw new Error('Electron 33+ is required for rounded Browser Workbench native views.');
  }
  record.view.setBorderRadius(borderRadius);
  try {
  ...
  return { ok: true, visible: true, session_id: sessionId, bounds: nextBounds, border_radius: borderRadius };
}
...
ipcMain.handle('browser-workbench:find-in-page', (_event, payload) => findInNativePage(payload || {}));
ipcMain.handle('browser-workbench:stop-find-in-page', (_event, payload) => stopFindInNativePage(payload || {}));
