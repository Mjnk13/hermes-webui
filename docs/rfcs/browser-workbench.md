...
The live Electron surface uses the DOM viewport's computed content-box bounds
and border radius through `View.setBorderRadius()`. Electron 33 or newer is
therefore required for the desktop shell. Keeping the native clip identical to
the snapshot's CSS clip prevents a sharp-to-rounded corner change when an
application overlay opens or closes.

During the snapshot-backed visibility handoff, the active native page disables
Electron background throttling. This keeps its Chromium compositor and Page
Visibility state active while the child view is hidden. Normal throttling is
restored after the live surface is visible and has had time to settle; the
restore is token-guarded so a rapid overlay reopen cannot background the page
while it is suppressed again.
